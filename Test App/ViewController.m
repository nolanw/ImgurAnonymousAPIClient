...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    // stream
    if (buttonIndex == 0) {
        self.uploadProgress = [[ImgurAnonymousAPIClient client] uploadImage:self.image withFilename:@"image.png" completionHandler:completionHandler];
    } else if (buttonIndex == 1) {
        self.uploadProgress = [[ImgurAnonymousAPIClient client] uploadAssetWithURL:self.assetURL filename:nil completionHandler:completionHandler];
    } else {
        NSData *data = UIImagePNGRepresentation(self.image);
        if (buttonIndex == 2) {
            self.uploadProgress = [[ImgurAnonymousAPIClient client] uploadImageData:data withFilename:@"image.png" completionHandler:completionHandler];
            ...
            self.uploadProgress = [[ImgurAnonymousAPIClient client] uploadImageFile:fileURL withFilename:nil completionHandler:completionHandler];
        } else if (buttonIndex == 4) {
            NSInputStream *stream = [NSInputStream inputStreamWithData:data];
            self.uploadProgress = [[ImgurAnonymousAPIClient client] uploadStreamedImage:stream length:data.length withFilename:@"image.png" completionHandler:completionHandler];
        } else {
